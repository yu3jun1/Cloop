...
Run ``python -m cloop.outcome_v2 --stage outcome|world|op|all``. The formal
...
from .artifacts import RunArtifacts, read_json, read_torch, upsert_jsonl, write_json, write_torch
from .config import load_config
from .data import DataBundle, DynamicsDataset, collate_dynamics, window_refs
from .metrics import mean_std, reliability_summary, spearman, survival_summary
from .outcome import PiecewiseExponentialHead
from .outcome_diagnostics import (
...
from .world import EnsembleWorldModel, OneStepDynamics, terminal_states, variant_spec
...
def _initialize(config: dict[str, Any], v2: dict[str, Any]) -> tuple[Any, ...]:
    ...
    artifacts = RunArtifacts(config["paths"]["output_root"], str(v2["run"]))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--paths", default="configs/server.yaml")
    parser.add_argument("--v2-config", default="configs/outcome_v2.yaml")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    RunArtifacts(config["paths"]["output_root"], str(v2["run"]))
    device = _resolve_device(config["project"]["device"])
    from .engine import validate_device_visibility
    validate_device_visibility(config, device)
    log_path = Path(config["paths"]["project_root"]) / "logs" / f"{v2['run']}.log"
