...
本轮实验使用独立入口 `src/cloop/v2/outcome_v2.py`、配置
`configs/v2/outcome_v2.yaml`、运行目录 `outputs/v2/outcome_v2_clarity_v1/` 和日志
`logs/v2/outcome_v2_clarity_v1.log`。旧的 E0–E5 diagnostics、正式 run、日志和
...
CUDA_VISIBLE_DEVICES=7 "$PY" -m cloop.v2.outcome_v2 \
  --config configs/v1/default.yaml --paths configs/v1/server.yaml \
  --v2-config configs/v2/outcome_v2.yaml --stage outcome

CUDA_VISIBLE_DEVICES=7 "$PY" -m cloop.v2.outcome_v2 \
  --config configs/v1/default.yaml --paths configs/v1/server.yaml \
  --v2-config configs/v2/outcome_v2.yaml --stage world

CUDA_VISIBLE_DEVICES=7 "$PY" -m cloop.v2.outcome_v2 \
  --config configs/v1/default.yaml --paths configs/v1/server.yaml \
  --v2-config configs/v2/outcome_v2.yaml --stage op
```
...
设计背景见[Outcome-v2 实验设计](cloop_outcome_v2_clarity_inspired_experiment_design.md)；
已有结果见[正式报告](../../outputs/v2/outcome_v2_clarity_v1/report.md)。
